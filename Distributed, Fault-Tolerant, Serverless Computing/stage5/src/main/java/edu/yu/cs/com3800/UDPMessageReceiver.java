...
public class UDPMessageReceiver extends Thread implements LoggingServer {
    ...
    private final LinkedBlockingQueue<Message> incomingElectionMessages, incomingGossipMessages;
        ...
        while (!this.isInterrupted()) {
            try {
                ...
                //end stage 5 logic
                	...
                	if(received.getMessageType() == MessageType.ELECTION) {
                		offerMsg(received, this.incomingElectionMessages);
                	} else if(received.getMessageType() == MessageType.GOSSIP) {
                		offerMsg(received, this.incomingGossipMessages);
                	}
                }
            }
    ...
    private static void offerMsg(Message msg, BlockingQueue<Message> queue) {
        //use interrupt-safe version, i.e. offer
    	boolean done= false;
    	while(!done) {
    		done= queue.offer(msg);
    	}
    }
}