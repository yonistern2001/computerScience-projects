...
public class ZooKeeperLeaderElection implements LoggingServer {
	...
	private boolean hasHigherVote(ElectionNotification vote) {
		final long proposedLeaderID = vote.getProposedLeaderID();
	...
	private boolean updateVote(ElectionNotification notification, Map<Long, Vote> votes) {
		if(notification.getProposedLeaderID() < this.proposedLeader || ServerState.OBSERVER.equals(notification.getState())) {
			return false;
		}
	...
	private Vote acceptElectionWinner(ElectionNotification n) {
		...
		ServerState state= this.myPeerServer.getServerId().equals(n.getProposedLeaderID()) ? ServerState.LEADING : ServerState.FOLLOWING;
